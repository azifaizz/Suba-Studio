...
const ValuesSection = () => {
  ...
  return (
    <section ref={containerRef} className="py-16 md:py-24 bg-[#111111] text-white overflow-hidden relative">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-6">
          {values.map((val, i) => {
            ...
            return (
              <div 
                key={i}
                className="value-card group bg-white/5 backdrop-blur-sm border border-white/10 p-6 md:p-8 rounded-2xl flex flex-col items-center text-center hover:bg-white/10 hover:-translate-y-2 md:hover:-translate-y-4 transition-all duration-500 shadow-xl hover:shadow-[#D4AF37]/20"
              >
                <div className="w-14 h-14 md:w-16 md:h-16 rounded-full bg-white/5 flex items-center justify-center mb-4 md:mb-6 group-hover:scale-110 group-hover:bg-[#D4AF37]/20 transition-all duration-500">
                  <Icon className="w-6 h-6 md:w-8 md:h-8 text-white/70 group-hover:text-[#D4AF37] transition-colors duration-500" />
                </div>
